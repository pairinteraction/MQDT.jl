...
default(dpi=400, fontfamily="sans-serif", titlefontsize=12, framestyle=:box, labels=false)

# LS g-factors
function g_L(J, L, S)
    return (J*(J+1) + L*(L+1) - S*(S+1)) / (2*J*(J+1))
end
function g_J(J, L, S)
    return 2 * (J*(J+1) - L*(L+1) + S*(S+1)) / (2*J*(J+1)) + g_L(J, L, S)
end
